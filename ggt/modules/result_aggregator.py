...
def calculate_cdf(data,n=1000,warnings=True):
    ...
    #Code from https://stackoverflow.com/questions/52221829/python-how-to-get-cumulative-distribution-function-for-continuous-data-values
...
def calculate_ci(data,n=1000,warnings=False):
    ...
    x_domain,cdf,mode,y_pdf,mode_arg = calculate_cdf(data,n=n,warnings=warnings)
    ...
    sig_ci = tuple([x_domain[lower_arg_arr[sig_ci_idx]],x_domain[upper_arg_arr[sig_ci_idx]]])

    twosig_ci_idx = find_idx_nearest_val(percentile_arr, 0.9545)
    twosig_ci = tuple([x_domain[lower_arg_arr[twosig_ci_idx]],x_domain[upper_arg_arr[twosig_ci_idx]]])
    
    threesig_ci_idx = find_idx_nearest_val(percentile_arr, 0.9973)
    threesig_ci = tuple([x_domain[lower_arg_arr[threesig_ci_idx]],x_domain[upper_arg_arr[threesig_ci_idx]]])
...
def unscale_preds(df_input,scaling_df_path=None,drop_old=True):
    ...
    if  "preds_custom_logit_bt" not in df.columns.values.tolist() or \
        "preds_ln_R_e_asec" not in df.columns.values.tolist() or \
        "preds_ln_total_flux_adus" not in df.columns.values.tolist():
        raise ValueError("This function is not appropriate for doing the transformatiuon")
    ...
    df["preds_bt"] = expit_custom(df["preds_custom_logit_bt"],scaling_array=scaling_array)
    df["preds_total_mag"] = -2.512 * np.log10(df["preds_total_flux_adus"]) + 27
    
    if drop_old is True:
        df = df.drop(columns=["preds_ln_R_e_asec","preds_ln_total_flux_adus","preds_custom_logit_bt"])
...
def file_loader(args):
        ...
        if unscale is True:
            new_df = unscale_preds(new_df,scaling_df_path=scaling_df_path,drop_old=drop_old)
        
        return new_df
        
def bayesian_inference_file_gobbler(data_dir,num=300,unscale=False,
                                    scaling_df_path=None,drop_old=True):
    ...
    if unscale is True:
        base_frame = unscale_preds(base_frame,scaling_df_path=scaling_df_path,
                                  drop_old=drop_old)
    ...
    with MultiPool() as pool:
        args = list(zip(range(num),[data_dir]*num,[unscale]*num,[scaling_df_path]*num,[drop_old]*num))
    ...
    for i in tqdm(range(num)):
        ...
        xarray_ds_img_number = np.array(data["object_id"][dict(inf_run_number=i)])
        
        xarray_indexes_of_new_df = np.nonzero(np.isin(xarray_ds_img_number,new_df_img_numbers))[0]
    
        for column_name in new_df.columns:
            if column_name.split('_')[0] == 'preds':
                data[column_name].loc[dict(inf_run_number=i)][xarray_indexes_of_new_df] = np.array(new_df[column_name])
...
def create_summary_df(data):
    ...
    index_total = data.sizes['index']
    inf_runs_total = data.sizes['inf_run_number']
    ...
    for pred_col in preds_cols:
        ...
        summary_df[pred_col + "_kurtosis"] = scipy.stats.kurtosis(preds_array,axis=0)
        print("Calculating Conf Ints for " + pred_col)
        with MultiPool() as pool:
            #mode,sig_ci,twosig_ci,threesig_ci = 
            output = np.array(list(tqdm(pool.imap(calculate_ci,preds_array.T), total=len(preds_array.T))),
                             dtype=object)
...
def main(data_dir,num,out_summary_df_path,
                      out_pdfs_path,
                      unscale,scaling_df_path,drop_old):
    """A function which performs all the above steps
    necessary to prepeare the data for analysis"""
    
    data = bayesian_inference_file_gobbler(data_dir,num=num,unscale=unscale,
                                          scaling_df_path=scaling_df_path,drop_old=drop_old)
    
    df, x_domains, y_pdfs = create_summary_df(data)
    df.to_csv(out_summary_df_path,index=False)
    
    #x_domains[:,0] --> This is how you can access the x_domains for the 0th galaxy
    #y_pdfs[:,0] --> This is how you can access the y_pdfs for the 0th galaxy
    #pdf_data = np.concatenate([x_domains[:,0],y_pdfs[:,0]],axis=0)  --> ALL pdf data for 0th galaxy
    #First 4 columns are domains; next 4 columns are pdfs
    #column order is preds_R_e_asec, preds_total_flux_adus, preds_bt, preds_total_mag
    ...
    args = list(zip(objids,out_pdfs_paths,x_0,x_1,x_2,x_3,y_0,y_1,y_2,y_3))
    
    print("Saving PDFs as Arrays")
    with MultiPool() as pool:
        outlist = list(tqdm(pool.imap(save_pdfs,args),total=len(objids)))
        
    if np.all(np.array(outlist)==0) == False:
        print("There was an issue in saving some .npy files. See the list below for non-zero elemetns")
        print(outlist)
